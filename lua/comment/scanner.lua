...
function M.scan(bufnr)
    ...
    for row, line in ipairs(lines) do
        -- Only the first match on a row gets `line_hl_group`: two keywords
        -- on one line would otherwise both set it on their own extmark,
        -- and which one wins would depend on unspecified extmark priority.
